...
- [卡牌前端交互设计](Design/zhengdao-cards/设计/卡牌前端交互设计.md) — **前端表现 + 交互层设计规格**(2026-05-22 启动): §0 卡牌化原则 + **§1 基础交互原语**(卡牌 / 标记 / 槽位=布局框架 / 牌桌·镜头)+ **§2 整体布局 + 信息架构**(有界槽位牌桌 / 上下塔罗外内二元 / 地点锚点簇→因果链 / 聚焦机制 / 拖拽投入). 落地 MVP 草案 §五;留后续交互细节小节(各卡视觉 / 投入鉴定 / 结果揭示 / 牌库平移 / 心性·NPC 呈现等)。
