...
- 当前阶段: **MVP 工程骨架就绪 → C 阶段 ✅ 完成 + Godot 4.6.2 升级 → D 阶段(MVP 1 期前置): §7.2 资源种类集合 ✅ 收官(2026-05-18, 心性 + 关系 + 孤注一掷三大机制落定), §7.1 普通事件撰写规格 / §7.3 普通事件对世界状态回馈 ⏳ → MVP 1 期实施**。
...
- [前端表现卡牌化_MVP草案](Design/zhengdao-cards/进度/前端表现卡牌化_MVP草案.md) — **草案 v3 + 工程启动 checklist** + **§7.2 资源体系议题已收官**(2026-05-18, 8 轮主对话):取消消耗类资源 / 心性系统重构(一种心性标记 + xinxing 切换规则 + 行为驱动) / 关系标记分配机制(未分配池 + NPC 卡 + 切换信任警戒) / 孤注一掷形态确定(失败后追加心性+关系再鉴定). 章节更新 §7.2 / §3.4 / §3.5 / §7.4 / §7.7 / §五. **§7.8 工程启动**: A/B/C 阶段 ✅ 完成 + Godot 4.6.2 升级 ✅. **D 阶段余下 §7.1 普通事件撰写规格 / §7.3 普通事件对世界状态回馈 ⏳**。
