...
- [前端表现卡牌化_MVP草案](Design/zhengdao-cards/进度/前端表现卡牌化_MVP草案.md) — **草案 v3 + 工程启动 checklist**(2026-05-15):核心承担模型(事件-选项-结果三层卡循环 + 选项层调度 + 鉴定+心性资源闭环 + 资源卡+标记 + 结果卡复用 outcome)+ §2.2 内核明确化(普通事件玩法 / chain 叙事承担分离)+ MVP 4 步迭代路径 + 14 条未决议题。**§7.8 工程启动**: A 阶段(Design 整理)+ B 阶段(本仓库初始化)**已完成**;**下一棒 C 阶段(引擎代码从 vibe-test 整体复制 + 启动验证)** → D 阶段(MVP 1 期前置 §7.1 普通事件撰写规格 / §7.2 资源种类集合 / §7.3 普通事件对世界状态回馈)。
