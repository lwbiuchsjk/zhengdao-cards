...
**两条并行线(互不阻塞,新会话挑一条起;单线程默认先起 Line A)**:

- **Line A — 前端骨架 + 架构验证**(= B②骨架 + B③ 实跑): 按 [卡牌前端交互设计](Design/zhengdao-cards/设计/卡牌前端交互设计.md)「技术基础」节搭 Node2D+Camera2D+Control 牌桌 + §1/§2 布局 + §3 揭示流程的**空壳**,用 **mock 数据**(对 §3.2 stub)验证 B③ 5 项实跑风险(拖拽优先级 / 聚焦漂移 / 帧率 / Safari / 动态槽位)。不依赖引擎内部。
- **Line B — 引擎扩展 + CSV 契约**: §五 引擎能力(选项层调度 / 资源标记系统 / 经验升阶 / xinxing / 人际)+ CSV 字段调整 + 同步契约三件套。不依赖前端。

**汇合 → §3.2 引擎 marker 契约**(⭐高优先级,Line B 完成后首要整合项): 把结算产出按 §3.2 stub 校准为真实"标记粒度账单",接通 Line A 揭示 → **核心循环(抽事件→选项→投入鉴定→揭示领取)用真数据跑通**。

**之后**: 各卡视觉 / 投入鉴定细节边原型边定 → **B④ 少量普通事件资产(§7.1)+ 数值标定**(需可跑循环 playtest)。

> **接口先行**:并行开始前先约定 §3.2 stub(粗略账单形状,见 [卡牌前端交互设计](Design/zhengdao-cards/设计/卡牌前端交互设计.md) §3.2),让 Line A 的 mock 与 Line B 的产出朝同一接口靠,§3.2 最终只做校准、避免大返工。
